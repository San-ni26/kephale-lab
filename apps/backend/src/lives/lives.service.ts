...
export class LivesService {
  ...
  async giftLive(userId: string, liveId: string, tokens: number, message?: string) {
    if (!tokens || tokens < 1) {
      throw new BadRequestException({ success: false, error: { message: 'Nombre de jetons invalide' } });
    }
    ...
    if (!live || (live.status !== 'LIVE' && live.status !== 'SCHEDULED')) {
      throw new NotFoundException({ success: false, error: { code: 'LIVE_NOT_ACTIVE', message: 'Live is not active' } });
    }

    const user = await this.prisma.user.findUnique({ where: { id: userId } });
    if (!user || user.tokenBalance < tokens) {
      throw new BadRequestException({ success: false, error: { code: 'INSUFFICIENT_TOKENS', message: 'Solde de jetons insuffisant' } });
    }

    const split = this.currencyService.calculateArtistSplit(tokens, 20);

    const donation = await this.prisma.$transaction(async (tx) => {
      const createdDonation = await tx.donation.create({
        data: { liveId, userId, tokens, message },
      });
      ...
      await tx.tokenTransaction.create({
        data: {
          ...
          amount: -tokens,
          type: 'SPEND_GIFT',
          description: `Cadeau en direct (${tokens} jetons) : ${live.title}`,
          balanceAfter: updatedUser.tokenBalance,
          metadata: {
            liveId,
            artistId: live.artistId,
            artistStageName: live.artist.stageName,
            split,
            message,
          },
        },
    ...
    if (live.artist.userId && live.artist.userId !== userId) {
      this.notificationsQueue.add('send-notification', {
        type: 'DONATION_RECEIVED',
        artistUserId: live.artist.userId,
        fromUserName: user.name || 'Un spectateur',
        tokens,
        liveId,
      }).catch(() => {});
    }
