...
export class AudioFingerprintService {
   ...
   */
  public static async downloadFromS3(s3Key: string, tmpDir: string): Promise<string> {
    const cleanKey = s3Key.startsWith(`${BUCKET_NAME}/`) ? s3Key.replace(`${BUCKET_NAME}/`, '') : s3Key;
   ...
   */
  public static async computeFileHash(filePath: string): Promise<string> {
   ...
   */
  public static async generateChromaprintFingerprint(audioFilePath: string): Promise<{
    fingerprint: string;
    duration: number;
  }> {
    try {
   ...
   */
  private static async generateFallbackFingerprint(audioFilePath: string): Promise<{
    fingerprint: string;
    duration: number;
  }> {
   ...
   */
  public static compareFingerprints(fp1: string, fp2: string): number {
    if (!fp1 || !fp2) return 0;
   ...
   *
  private static decodeChromaprintToArray(fingerprint: string): number[] {
    try {
  ...
  */
   ...
   */
  private static popcount32(n: number): number {
   ...
   */
  public static async queryACRCloud(audioFilePath: string): Promise<{
    title: string;
    artist: string;
    score: number;
    externalId?: string;
   ...
   */
  public static async queryAudD(audioFilePath: string): Promise<{
    title: string;
    artist: string;
    score: number;
  } | null> {
   ...
   */
  public static calculateTextSimilarity(str1: string, str2: string): number {
   ...
   */
  public static async checkUserStrikes(userId: string): Promise<{
    blocked: boolean;
   ...
   */
  public static async analyzeAndDetectCopyright(params: {
    userId: string;
    trackId?: string;
    audioTitle?: string;
    videoS3Key?: string;
        ...
        if (!result.isAuthorized) {
          return {
            ...
            message: result.isAuthorized
              ...
              : `⚠️ Droits d'auteur détectés : "${textMatch.track.title}" de ${textMatch.track.artist.stageName}. ${result.message}`,
          };
        }
   ...
   */
  private static async performAudioAnalysis(
    videoS3Key: string,
    paidTracks: TrackWithArtist[],
    userId: string
      ...
      if (bestMatchTrack && maxSimilarity >= CHROMAPRINT_THRESHOLD) {
        ...
        return {
          ...
          message: result.isAuthorized
            ...
            : `⚠️ Musique protégée détectée par analyse acoustique : "${bestMatchTrack.title}" de ${bestMatchTrack.artist.stageName}. ${result.message}`,
        };
      }
        ...
        if (matchedTrack) {
          ...
          return {
            ...
            message: result.isAuthorized
              ...
              : `⚠️ Musique protégée détectée (ACRCloud) : "${matchedTrack.title}" de ${matchedTrack.artist.stageName}. ${result.message}`,
          };
        }
      }
        ...
        if (matchedTrack) {
          ...
          return {
            ...
            message: result.isAuthorized
              ...
              : `⚠️ Musique protégée détectée (AudD) : "${matchedTrack.title}" de ${matchedTrack.artist.stageName}. ${result.message}`,
          };
        }
      }
   ...
   */
  private static matchAPIResultToKephaleCatalog(
   ...
   */
  private static async verifyRightsForTrack(
    userId: string,
    track: {
      id: string;
      title: string;
    ...
    const userArtist = await prisma.artistProfile.findUnique({ where: { userId } });
   ...
   */
  private static async getAllTracksWithCache(): Promise<TrackWithArtist[]> {
    ...
    try {
      await redis.setex(cacheKey, CATALOG_CACHE_TTL, JSON.stringify(tracks));
    } catch {
      // Ignore cache errors
    }
   ...
   */
  public static async invalidateCatalogCache(): Promise<void> {
    try {
      await redis.del('kephale:tracks_catalog');
      await redis.del('kephale:paid_tracks_catalog'); // Legacy key cleanup
    } catch {
      // Ignore
    }
  }
   ...
   */
  public static async postUploadVerification(videoId: string): Promise<{
    violation: boolean;
    trackId?: string;
    method?: string;
  }> {
    ...
    const userId = video.userId || (video.artistId
      ? (await prisma.artistProfile.findUnique({ where: { id: video.artistId }, select: { userId: true } }))?.userId
      : null);
   ...
   */
  public static async generateAndSaveTrackFingerprint(trackId: string): Promise<void> {
    const track = await prisma.track.findUnique({
      where: { id: trackId },
